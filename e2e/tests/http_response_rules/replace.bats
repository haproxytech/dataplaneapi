...
@test "http_request_rules: Replace all HTTP Response Rule of frontend" {
    resource_put "$_RES_RULES_BASE_PATH" "data/replace-all.json" "parent_type=frontend&parent_name=test_frontend"
    assert_equal "$SC" 202

    resource_get "$_RES_RULES_BASE_PATH" "parent_name=test_frontend&parent_type=frontend"
    assert_equal "$SC" 200
...
@test "http_request_rules: Replace all HTTP Reponse Rule of backend" {
	resource_put "$_RES_RULES_BASE_PATH" "data/replace-all.json" "parent_type=backend&parent_name=test_backend"
    assert_equal "$SC" 202

    resource_get "$_RES_RULES_BASE_PATH" "parent_name=test_backend&parent_type=backend"
    assert_equal "$SC" 200
    assert_equal "$(get_json_path "${BODY}" ". | length")" 3
    assert_equal "$(get_json_path "$BODY" ".")" "$(get_json_path "$(cat "$BATS_TEST_DIRNAME/data/replace-all.json")" ".")"
}
