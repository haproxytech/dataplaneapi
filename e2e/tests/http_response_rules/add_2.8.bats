...
@test "http_response_rules: Add a new HTTP Response Rule track-sc to backend" {
  ...
  resource_post "$_RES_RULES_BASE_PATH/0" "data/post-track-sc.json" "parent_type=backend&parent_name=test_sticksc&force_reload=true"
	assert_equal "$SC" 201
...
@test "http_response_rules: Fail - Add a new HTTP Response Rule track-sc to backend - when track_sc_stick_counter is missing" {
  ...
  resource_post "$_RES_RULES_BASE_PATH/0" "data/post-track-sc-fail.json" "parent_type=backend&parent_name=test_sticksc&force_reload=true"
	assert_equal "$SC" 400
  fi
}
