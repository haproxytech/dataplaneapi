...
@test "http_response_rules: Delete a HTTP Response Rule from frontend" {
	run dpa_curl DELETE "/services/haproxy/configuration/http_response_rules/0?parent_type=frontend&parent_name=test_frontend&force_reload=true&version=$(version)"
	assert_success
...
@test "http_response_rules: Delete a HTTP Response Rule from backend" {
	run dpa_curl DELETE "/services/haproxy/configuration/http_response_rules/0?parent_type=backend&parent_name=test_backend&force_reload=true&version=$(version)"
	assert_success
