...
setup() {
  ...
  run docker exec -d ${DOCKER_CONTAINER_NAME} sh -c 'mkdir /tmp/maps'
  run docker cp "${E2E_DIR}/tests/runtime_maps_entries/data/mapfile1.map" ${DOCKER_CONTAINER_NAME}:/tmp/maps
  run docker cp "${E2E_DIR}/tests/runtime_maps_entries/data/mapfile2.map" ${DOCKER_CONTAINER_NAME}:/tmp/maps
  ...
  run docker exec -d ${DOCKER_CONTAINER_NAME} /bin/sh -c "CI_DATAPLANE_RELOAD_DELAY_OVERRIDE=1 dataplaneapi --log-level=debug --maps-dir=/tmp/maps --userlist-file=/etc/haproxy/userlist.cfg --host=0.0.0.0 --port=8080 --reload-cmd='kill -SIGUSR2 1' --restart-cmd='kill -SIGUSR2 1' --haproxy-bin=/usr/local/sbin/haproxy --log-to=file"
  assert_success
...
teardown() {
  run docker exec -d ${DOCKER_CONTAINER_NAME} sh -c 'rm -rf /tmp/maps'
