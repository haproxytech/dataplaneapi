...
@test "tcp_checks: Return connect TCP check from a backend" {
	resource_get "$_TCP_CHECKS_CERTS_BASE_PATH/0" "parent_type=backend&parent_name=test_backend_get"
	assert_equal "$SC" 200
	assert_equal "$(get_json_path "$BODY" ".data.action")" "connect"
}

@test "tcp_checks: Return send TCP check from a backend" {
	resource_get "$_TCP_CHECKS_CERTS_BASE_PATH/1" "parent_type=backend&parent_name=test_backend_get"
	assert_equal "$SC" 200
	assert_equal "$(get_json_path "$BODY" ".data.action")" "send"
}

@test "tcp_checks: Return expect TCP check from a backend" {
	resource_get "$_TCP_CHECKS_CERTS_BASE_PATH/2" "parent_type=backend&parent_name=test_backend_get"
	assert_equal "$SC" 200
	assert_equal "$(get_json_path "$BODY" ".data.action")" "expect"
}

@test "tcp_checks: Return send-binary TCP check from a backend" {
	resource_get "$_TCP_CHECKS_CERTS_BASE_PATH/3" "parent_type=backend&parent_name=test_backend_get"
	assert_equal "$SC" 200
...
@test "tcp_checks: Return a non existing TCP check from a backend" {
  resource_get "$_TCP_CHECKS_CERTS_BASE_PATH/1000" "parent_type=backend&parent_name=test_backend_get"
  assert_equal "$SC" 404
}

@test "tcp_checks: Return a TCP check from a non existing backend" {
  resource_get "$_TCP_CHECKS_CERTS_BASE_PATH/0" "parent_type=backend&parent_name=unknown"
  assert_equal "$SC" 400
}

@test "tcp_checks: Return a TCP check from a frontend" {
  resource_get "$_TCP_CHECKS_CERTS_BASE_PATH/0" "parent_type=frontend&parent_name=unknown"
  assert_equal "$SC" 422
}
