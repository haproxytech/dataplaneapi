...
setup() {
	...
	[ "${SC}" = 201 ]
	read -r SC _ < <(auth_curl POST "/v2/services/haproxy/configuration/http_request_rules?parent_type=frontend&parent_name=test_frontend&force_reload=true&version=$(version)" "@${E2E_DIR}/tests/http_request_rules/if.json")
	[ "${SC}" = 201 ]
	...
	[ "${SC}" = 201 ]
	read -r SC _ < <(auth_curl POST "/v2/services/haproxy/configuration/http_request_rules?parent_type=backend&parent_name=test_backend&force_reload=true&version=$(version)" "@${E2E_DIR}/tests/http_request_rules/if.json")
	[ "${SC}" = 201 ]
}
...
@test "Return an array of all HTTP Request Rules from frontend" {
	read -r SC BODY < <(auth_curl GET "/v2/services/haproxy/configuration/http_request_rules?parent_type=frontend&parent_name=test_frontend")
	[ "${SC}" = 200 ]
...
@test "Return one HTTP Request Rule from backend" {
	read -r SC BODY < <(auth_curl GET "/v2/services/haproxy/configuration/http_request_rules?parent_type=backend&parent_name=test_backend")
	[ "${SC}" = 200 ]
	[ "$(get_json_path "${BODY}" ".data | length")" = 2 ]
	[ "$(get_json_path "${BODY}" ".data[0].cond")" = "if" ]
	[ "$(get_json_path "${BODY}" ".data[0].cond_test")" = "{ src 10.0.0.0/8 }" ]
