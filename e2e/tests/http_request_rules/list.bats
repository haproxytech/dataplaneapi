...
setup() {
	...
	dpa_curl_status_body '$output'
	assert_equal $SC 201
	run dpa_curl POST "/services/haproxy/configuration/http_request_rules?parent_type=frontend&parent_name=test_frontend&force_reload=true&version=$(version)" "../http_request_rules/if.json"
	assert_success
	...
	dpa_curl_status_body '$output'
	assert_equal $SC 201
	run dpa_curl POST "/services/haproxy/configuration/http_request_rules?parent_type=backend&parent_name=test_backend&force_reload=true&version=$(version)" "../http_request_rules/if.json"
	assert_success
...
@test "http_request_rules: Return an array of all HTTP Request Rules from frontend" {
	run dpa_curl GET "/services/haproxy/configuration/http_request_rules?parent_type=frontend&parent_name=test_frontend"
	assert_success
...
@test "http_request_rules: Return one HTTP Request Rule from backend" {
	run dpa_curl GET "/services/haproxy/configuration/http_request_rules?parent_type=backend&parent_name=test_backend"
	assert_success
