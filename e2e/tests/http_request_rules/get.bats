...
@test "http_request_rules: Return one HTTP Request Rule from frontend" {
	run dpa_curl GET "/services/haproxy/configuration/http_request_rules/0?parent_type=frontend&parent_name=test_frontend"
	assert_success
...
@test "http_request_rules: Return one HTTP Request Rule from backend" {
	run dpa_curl GET "/services/haproxy/configuration/http_request_rules/0?parent_type=backend&parent_name=test_backend"
	assert_success
