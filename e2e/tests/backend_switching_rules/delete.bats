...
setup() {
	...
	[ "${SC}" = 201 ]
	read -r SC _ < <(auth_curl POST "/v2/services/haproxy/configuration/backend_switching_rules?frontend=test_frontend&force_reload=true&version=$(version)" "@${E2E_DIR}/tests/backend_switching_rules/if.json")
	[ "${SC}" = 201 ]
}
