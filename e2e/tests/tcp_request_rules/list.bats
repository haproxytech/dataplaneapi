...
setup() {
	...
	[ "${SC}" = 201 ]
	read -r SC _ < <(auth_curl POST "/v2/services/haproxy/configuration/tcp_request_rules?parent_type=frontend&parent_name=test_frontend&force_reload=true&version=$(version)" "@${E2E_DIR}/tests/tcp_request_rules/reject.json")
	[ "${SC}" = 201 ]
	...
	[ "${SC}" = 201 ]
	read -r SC _ < <(auth_curl POST "/v2/services/haproxy/configuration/tcp_request_rules?parent_type=backend&parent_name=test_backend&force_reload=true&version=$(version)" "@${E2E_DIR}/tests/tcp_request_rules/reject.json")
	[ "${SC}" = 201 ]
}
...
@test "Return an array of all TCP Request Rules from frontend" {
	read -r SC BODY < <(auth_curl GET "/v2/services/haproxy/configuration/tcp_request_rules?parent_type=frontend&parent_name=test_frontend")
	[ "${SC}" = 200 ]
...
@test "Return one TCP Request Rule from backend" {
	read -r SC BODY < <(auth_curl GET "/v2/services/haproxy/configuration/tcp_request_rules?parent_type=backend&parent_name=test_backend")
	[ "${SC}" = 200 ]
	[ "$(get_json_path "${BODY}" ".data | length")" = 2 ]
	[ "$(get_json_path "${BODY}" ".data[0].action")" = "accept" ]
	[ "$(get_json_path "${BODY}" ".data[0].cond_test")" = "{ src 192.168.0.0/16 }" ]
