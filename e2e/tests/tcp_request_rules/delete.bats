...
@test "tcp_request_rules: Delete a TCP Request Rule from frontend" {
	run dpa_curl DELETE "/services/haproxy/configuration/tcp_request_rules/0?parent_type=frontend&parent_name=test_frontend&force_reload=true&version=$(version)"
	assert_success
...
@test "tcp_request_rules: Delete a TCP Request Rule from backend" {
	run dpa_curl DELETE "/services/haproxy/configuration/tcp_request_rules/0?parent_type=backend&parent_name=test_backend&force_reload=true&version=$(version)"
	assert_success
