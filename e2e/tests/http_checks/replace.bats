...
@test "http_checks: Fail replacing an unexisting HTTP Check of backend" {
	PARENT_NAME="test_backend"
	resource_put "$_BACKEND_BASE_PATH/$PARENT_NAME/http_checks/1000" "data/put.json" "force_reload=true"
	assert_equal "$SC" 404
}

@test "http_checks: Fail replacing a HTTP Check of frontend" {
	resource_put "$_FRONTEND_BASE_PATH/test_frontend/http_checks/1000" "data/put.json" "force_reload=true"
	assert_equal "$SC" 404
}
...
@test "http_checks: Replace all HTTP Checks of backend (>=2.2)" {
    ...
    then
	PARENT_NAME="test_backend"
	resource_put "$_BACKEND_BASE_PATH/$PARENT_NAME/http_checks" "data/replace-all.json"
	assert_equal "$SC" 202
...
@test "http_checks: Replace all HTTP Checks for defaults (>=2.2)" {
    ...
    then
	PARENT_NAME="mydefaults"
	resource_put "$_DEFAULTS_BASE_PATH/$PARENT_NAME/http_checks" "data/replace-all.json"
	assert_equal "$SC" 202
