...
func (o *ReplaceServerURL) Build() (*url.URL, error) {
	...
	var _path = "/services/haproxy/configuration/servers/{name}"

	name := o.Name
	if name != "" {
		_path = strings.Replace(_path, "{name}", name, -1)
	} else {
		return nil, errors.New("name is required on ReplaceServerURL")
	}
...
func (o *ReplaceServerURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on ReplaceServerURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on ReplaceServerURL")
	}
