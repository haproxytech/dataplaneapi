...
func (o *ReplaceFilterURL) Build() (*url.URL, error) {
	...
	var _path = "/services/haproxy/configuration/filters/{index}"

	index := swag.FormatInt64(o.Index)
	if index != "" {
		_path = strings.Replace(_path, "{index}", index, -1)
	} else {
		return nil, errors.New("index is required on ReplaceFilterURL")
	}
...
func (o *ReplaceFilterURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on ReplaceFilterURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on ReplaceFilterURL")
	}
