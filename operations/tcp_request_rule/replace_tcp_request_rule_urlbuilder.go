...
func (o *ReplaceTCPRequestRuleURL) WithBasePath(bp string) *ReplaceTCPRequestRuleURL {
	o.SetBasePath(bp)
	return o
}
...
func (o *ReplaceTCPRequestRuleURL) Build() (*url.URL, error) {
	...
	var _path = "/services/haproxy/configuration/tcp_request_rules/{index}"

	index := swag.FormatInt64(o.Index)
	if index != "" {
		_path = strings.Replace(_path, "{index}", index, -1)
	} else {
		return nil, errors.New("index is required on ReplaceTCPRequestRuleURL")
	}
...
func (o *ReplaceTCPRequestRuleURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on ReplaceTCPRequestRuleURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on ReplaceTCPRequestRuleURL")
	}
