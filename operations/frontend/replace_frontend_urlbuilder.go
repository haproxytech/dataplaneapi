...
func (o *ReplaceFrontendURL) Build() (*url.URL, error) {
	...
	var _path = "/services/haproxy/configuration/frontends/{name}"

	name := o.Name
	if name != "" {
		_path = strings.Replace(_path, "{name}", name, -1)
	} else {
		return nil, errors.New("name is required on ReplaceFrontendURL")
	}
...
func (o *ReplaceFrontendURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on ReplaceFrontendURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on ReplaceFrontendURL")
	}
