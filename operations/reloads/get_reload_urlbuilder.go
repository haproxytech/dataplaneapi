...
func (o *GetReloadURL) Build() (*url.URL, error) {
	...
	var _path = "/services/haproxy/reloads/{id}"

	id := o.ID
	if id != "" {
		_path = strings.Replace(_path, "{id}", id, -1)
	} else {
		return nil, errors.New("id is required on GetReloadURL")
	}
...
func (o *GetReloadURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on GetReloadURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on GetReloadURL")
	}
