...
# We only want the embedded specification (embedded_spec.go). Everything else
# that go-swagger emits is generated into a throwaway directory and discarded;
# the chi-based server is generated by oapi-codegen (make specification).
...
echo " ---> copy embedded_spec.go to destination"
cp $DST_DIR/dataplaneapi/embedded_spec.go embedded_spec.go
